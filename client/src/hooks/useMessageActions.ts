...
export function useMessageActions({ role, messageId, content, conversationId }: UseMessageActionsParams) {
  ...
  const handleDelete = useCallback(() => {
    ...
    tauriClient.sendRpc('message.delete', { conversation_id: resolvedConvId ?? activeConvId, message_id: messageId });
  ...
  const handleRetry = useCallback(() => {
    ...
    tauriClient.sendRpc('message.retry', { conversation_id: resolvedConvId, message_id: messageId });
  ...
  const handleAdopt = useCallback(() => {
    if (!resolvedConvId) return;
    tauriClient.sendRpc('message.adopt', { conversation_id: resolvedConvId, message_id: messageId });
    showToast('Adopted');
  }, [resolvedConvId, messageId]);
  ...
  const handleBranchConfirm = useCallback((label: string) => {
    ...
    const parentBranchId = activeConvId?.startsWith('branch:')
      ? activeBranchId ?? null
      : null;
    tauriClient.sendRpc('branch.create', {
