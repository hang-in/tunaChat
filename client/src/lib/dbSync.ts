...
export function syncTokenUsage(convId: string, inputTokens: number, outputTokens: number, costUsd: number) {
  fire(async () => {
    const db = await getDb();
    db?.addTokenUsage(convId, inputTokens, outputTokens, costUsd);
  });
}

export function syncTraceLog(entry: {
  conversationId: string; eventType: string; agentRole?: string;
  detail?: string; tokenCount?: number; costUsd?: number;
}) {
  fire(async () => {
    const db = await getDb();
    db?.insertTraceLog(entry);
  });
}
