  ...
  return (
    <div className="flex-1 flex items-center justify-center">
      <div className="text-center max-w-[260px] space-y-3">
        {!conv ? (
          <>
            <div className="text-[14px] font-semibold text-foreground/70">tunaChat</div>
...
export function ChatArea() {
  ...
  useEffect(() => {
    ...
    if (!contextLoadedConvs.has(activeConversationId)) {
      tauriClient.sendRpc('project.context', {
    ...
    if (conv?.source && conv.source !== 'tunachat') {
      histParams.source = conv.source;
    }
    tauriClient.sendRpc('conversation.history', histParams);
