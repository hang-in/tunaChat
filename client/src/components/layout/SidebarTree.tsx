...
function ProjectRow({ node, open, toggle }: { node: SidebarNode; open: boolean; toggle: () => void }) {
  ...
  const handleClick = () => {
    if (node.projectKey) {
      ...
      if (isConnected) {
        tauriClient.sendRpc('conversation.list', { project: node.projectKey });
        tauriClient.sendRpc('branch.list.json', { project: node.projectKey });
      }
    }
  ...
  const handleNewSession = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (node.projectKey) {
      tauriClient.sendRpc('conversation.create', { conversation_id: crypto.randomUUID(), project: node.projectKey });
    }
  };
...
function SessionRow({ node, open, toggle, hasChildren }: {
  ...
  const handleDelete = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!isRunning) tauriClient.sendRpc('conversation.delete', { conversation_id: conv.id }).catch(console.error);
  };
