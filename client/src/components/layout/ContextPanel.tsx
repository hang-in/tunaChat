...
function CodeSearchTab() {
  ...
  const doSearch = useCallback((q: string) => {
    ...
    tauriClient.searchCode(q.trim(), ctx.project);
  }, [ctx?.project]);
...
export function ContextPanel() {
  ...
  useEffect(() => {
    ...
    tauriClient.sendRpc('project.context', params);
    // rawq code map 요청 (overview 탭 구조 뷰용)
    tauriClient.getCodeMap(activeProjectKey);
  ...
  useEffect(() => {
    ...
    if (activeTab === 'branches') {
      tauriClient.sendRpc('branch.list.json', params);
    } else if (activeTab === 'memory') {
      tauriClient.sendRpc('memory.list.json', params);
    }
