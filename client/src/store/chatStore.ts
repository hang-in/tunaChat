...
const LS_KEY_PROJECT = 'tunachat:lastProjectKey';
const LS_KEY_CONV = 'tunachat:lastConvId';
...
export interface Conversation {
  ...
  createdAt: number;
  source?: 'tunachat' | 'mattermost' | 'slack';
}
...
interface ChatState {
  ...
  loadConversations: (convs: Array<{ id: string; projectKey: string; label: string; created_at: number; source?: string; engine?: string; model?: string; persona?: string; triggerMode?: string }>, fromDb?: boolean) => void;
  ...
  /** Update progressContent without touching content (for streaming progress display) */
  updateProgress: (convId: string, msgId: string, progressContent: string) => void;
...
export const useChatStore = create<ChatState>((set, get) => ({
    ...
    for (const c of convs) {
      ...
      if (existing) {
        ...
        newConvs[c.id] = {
          ...
          persona: fromDb ? (c.persona ?? existing.persona) : (existing.persona ?? c.persona),
          triggerMode: fromDb ? (c.triggerMode ?? existing.triggerMode) : (existing.triggerMode ?? c.triggerMode),
        };
      } else {
        newConvs[c.id] = {
          id: c.id,
          ...
          source: (c.source as Conversation['source']) ?? 'tunachat',
          engine: c.engine,
          model: c.model,
          persona: c.persona,
  ...
  updateProgress: (convId, msgId, progressContent) => set((state) => {
    const arr = state.messages[convId] ?? [];
    const updated = arr.map(m => {
      if (m.id !== msgId) return m;
      // Append to existing progress (rolling log)
      const prev = m.progressContent || '';
      const newProgress = prev ? prev + '\n' + progressContent : progressContent;
      return { ...m, progressContent: newProgress, content: newProgress };
    });
    return { messages: { ...state.messages, [convId]: updated } };
  }),
  ...
  clearDraft: (convId) => set(s => { const { [convId]: _, ...rest } = s.drafts; return { drafts: rest }; }),
