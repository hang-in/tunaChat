...
const SS_KEY_BRANCH = 'tunachat:branchPanel';
...
export const useSystemStore = create<SystemState>((set) => ({
  ...
  recentServers: (() => {
    try {
      const raw = localStorage.getItem('tunachat:recentServers');
  ...
  addRecentServer: (url) => set((s) => {
    ...
    try { localStorage.setItem('tunachat:recentServers', JSON.stringify(updated)); } catch { /* ignore */ }
